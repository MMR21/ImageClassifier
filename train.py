    ...
    for e in range(epochs):
        ...
        for ii, (inputs, labels) in enumerate(dataloaders[0]): # 0 = train
            ...
            if steps % print_every == 0:
                ...
                for ii, (inputs2,label2) in enumerate(dataloaders[1]): # 1 = validation
                    optimizer.zero_grad()
                    ...
                    # if torch.cuda.is_available(): # commentingout to work with later possibly
                         #inputs2, labels2 = inputs2.to('cuda') , labels2.to('cuda') # use cuda
                         #model.to('cuda:0') # use cuda
...
def main():
    print("hello") # just to make sure I can see something before it takes forever to train
    args = parse_args()
    ...
    validation_transforms = transforms.Compose([transforms.Resize(256), transforms.CenterCrop(224),               transforms.ToTensor(),
                                                transforms.Normalize([0.485, 0.456, 0.406],
                                                                     [0.229, 0.224, 0.225])])
    # the spacing on this looks off but it is just the workspace, do not pay it any mind
    ...
    if args.arch == "vgg13":
        ...
        classifier = nn.Sequential(OrderedDict([
                                  ...
                                  ('drop' , nn.Droupout(p=0.6)),
                                  ('relu' , nn.ReLU()),
                                  ('fc2' ,nn.Linear(500, 102)),
                                  ('output', nn.LogSoftmax(dim=1))]))
        
    model.classifier = classifier
    criterion = nn.NLLLoss() # using criterion and optimizer similar to pytorchlectures (densenet)
    optimizer = optim.Adam(model.classifier.parameters(), lr=float(args.learning_rate))
    epochs = int(args.epochs)
    class_index = image_datasets[0].class_to_idx
    gpu = args.gpu # get the gpu settings
    train(model, criterion, optimizer, dataloadrs, epochs, gpu)
