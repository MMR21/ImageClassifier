...
def parse_args():
    ...
    parser.add_argument('--filepath', dest='filepath', default='flowers/test/1/image_06734.jpg')# use a default filepath to a primrose image
...
def predict(image_path, model, topk=3, gpu='gpu'):
    ''' Get probability values (indeces) and respictive flower classes.
    '''
    ...
    probs = np.array(probaility.topk(topk)[0][0])
